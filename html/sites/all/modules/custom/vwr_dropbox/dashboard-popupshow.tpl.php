...
<form  name="iconform" id="iconform">
	<div class="pages-list">
		...
		?>
		<input type="checkbox" name="names"  style="display:none" class="default" value="Dropbox" <?php if($select_dropbox==1) {echo "checked";}?>>
		<span style="color:#0024C0;padding-left:5px;display:none" ><?php echo htmlspecialchars($title);?></span><br/> 
		<?php
		if(is_vwr_user_role()) {
			$bulk_title = "View Reports";
		?>
			<input type="checkbox" name="names" class="default"  style="display:none" value="Supplier Org" <?php if($select_supplierorg==1) {echo "checked";}?>>
			<span style="color:#0024C0;padding-left:5px;display:none">Supplier Org</span><br/> 
			<input type="checkbox" name="names" class="default" style="display:none" value="Users" <?php if($select_users==1) {echo "checked";}?>>
		...
		?>
		<input type="checkbox" name="names" class="default" style="display:none" value="<?php echo $bulk_title;?>" <?php if($select_bulk==1) {echo "checked";}?>>
			...
			$displayinfo="style='display:none'";
				...
				?>
					<div  <?php echo $displayinfo;?>>
						<input type="checkbox" id="<?php echo $record->category_id;?>" name="names" class="category" value="cat#<?php echo $record->category_id;?>#regionid#<?php echo $record->region_id;?>" <?php if(in_array($record->category_id." ".$record->region_id,$cat_listings)) {echo "checked";} ?> >
		                ...
		                echo htmlspecialchars($record->short_name)."&nbsp;<b>".$regionactualname."</b>";
			...
			$subresult = db_query('SELECT t.topic_id,t.topic_name,t.short_name,mr.region_shortname,mr.region_id FROM 
					...
					if($subresult) {
						...
						if(!in_array($subrecord->region_id,$currentregionslisting))
						{
							...
							?><div  <?php echo $displayinfo;?>>
								<input type="checkbox" id="<?php echo $subrecord->topic_id;?>" name="names" class="subcategory" value="subcat#<?php echo $subrecord->topic_id;?>#regionid#<?php echo $subrecord->region_id;?>" <?php if(in_array($subrecord->topic_id." ".$record->region_id,$subcat_listings)) {echo "checked";}?> >
								<span style="color:#0024C0;padding-left:5px;">	
								<?php 
								...
								echo htmlspecialchars($subrecord->short_name)."&nbsp;<b>".$subregionactualname."</b>";
	...
	$count = $select_details->rowCount();
					...
					foreach ($select_details as $record_view) {
						...
						?>
							...
							<input type="checkbox" id="<?php echo $record_view->id; ?>" name="names" class="dropboxes" value="dropboxes#<?php echo $record_view->id;?>#regionid#<?php echo $record_view->region_id;?>" <?php if(in_array($record_view->id." ".$record_view->region_id,$dropboxes_listings)) {echo "checked";}?>>								
							<span style="color:#0024C0;padding-left:5px;">
								
								<?php
								/*$dropboxregionscount=db_select("dropbox_regions","dr")
								->fields("dr",array('dropbox_region_id','region_id','dropbox_id','status'))
								->condition('dr.dropbox_id', $record_view->id,'=')
								...
								echo htmlspecialchars($record_view->title)."&nbsp;<b>".$dropboxregionactualname."</b>";
