	...
	<div class="pop_cont cat_tpls">
		...
		<div class="poptxt cat_upld">
			...
			<input type="text" value="<?php echo  htmlentities($result->hyperlink_name); ?>" id="hyperlinkname" />
			...
			<input type="file" accept="image/*" value="<?php echo htmlspecialchars($hyperlink_image); ?>" id="hyperlinkfile" name="hyperlinkfile" onchange="initiateScan('category','<?php echo 'category/'.htmlspecialchars($hyperlink_id); ?>')">
			<font style="float:right;margin-top:-20px;">Limit 500MB</font></input>
			<?php
			if ($hyperlink_id && $hyperlink_image) {
			?>
				<span class="img_existed"><img alt="<?php echo htmlspecialchars($hyperlink_image); ?>" title="<?php echo htmlspecialchars($hyperlink_image); ?>" src="<?php echo '/sites/default/files/topic/' . htmlspecialchars($hyperlink_image); ?>" height="30" width="32" /></span>
			<?php
			}
			?>
		...
		<div class="popBut1">
			...
			<input type="submit"  class="button" value="<?php echo htmlspecialchars($action_value); ?>" onClick="validateAllHyperlinks('<?php echo htmlspecialchars($hyperlink_id); ?>','category','<?php echo 'category/' . htmlspecialchars($hyperlink_id); ?>', '<?php echo htmlspecialchars($params); ?>')" />
