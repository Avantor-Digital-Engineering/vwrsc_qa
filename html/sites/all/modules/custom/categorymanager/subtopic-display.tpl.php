<?php drupal_add_js(drupal_get_path('module', 'categorymanager') . '/js/modalbox_dropbox.js');
	...
	?>
	<?php $parent_topic_id = $topic_id = $cat_id = 0;
	if (arg(5) && is_numeric(base64_decode(arg(5)))) {
		$topic_id = base64_decode(arg(5));
	}
	if (arg(3) && is_numeric(base64_decode(arg(3)))) {
		$parent_topic_id = base64_decode(arg(3));
	}
				...
				if (in_array($region_id, $cookie_regions)) {
					if (!is_vwr_user_role()) {
						...
						if ($resultset->supplier_org_id > 0) {
							...
							if ($isglobal <= 1) {
								$topicprocess_id =	getcontentregionaccess($cat_id, $topic_id, 2, $region_id, $resultset->supplier_org_id);
								if (empty($topicprocess_id)) {
									continue;
								}
							}
						}
	...
	if ($record) {
		$action_url = 'category/' . base64_encode($cat_id) . '/topic/' . base64_encode($parent_topic_id);
		...
		$parent_data = db_query('SELECT topic_name, expiry_date FROM {topic} where topic_id=:topic_id && category_id=:cat_id', array(':topic_id' => $parent_topic_id, ':cat_id' => $cat_id))->fetchObject();
	...
	?>
		...
		<div class="right_cont">
			<div class="inbread">
				<a href="<?php echo base_path(); ?>">VWR Home</a>&gt;<a href="<?php echo base_path() . 'category/' . $cat_id; ?>"><?php echo (strlen($cat_name) > 25) ? substr(htmlspecialchars($cat_name), 0, 23) . '...' : htmlspecialchars($cat_name); ?></a>&gt;<a href="<?php echo base_path() . $action_url; ?>"><?php echo (strlen($parent_data->topic_name) > 25) ? substr(htmlspecialchars($parent_data->topic_name), 0, 23) . '...' : htmlspecialchars($parent_data->topic_name); ?></a>&gt;<span><?php echo (strlen($record->topic_name) > 25) ? substr(htmlspecialchars($record->topic_name), 0, 23) . '...' : htmlspecialchars($record->topic_name); ?></span>
			</div>
			<div class="cat_cont">
				<div class="cat_top">
					<h3 class="<?php echo ($record->expiry_date < (strtotime(date("m/d/Y")))) ? 'doc_title_exp' : ''; ?>"><?php echo htmlspecialchars($record->topic_name); ?></h3>
					<?php if (user_access('add edit delete subtopic') && $edit_access) { ?>
						<a onClick="editCategoryTopic('<?php echo $topic_id; ?>','subtopic','<?php echo $action_url . '/subtopic/edit/' . $topic_id; ?>');" href="javascript:void(0);"><img src="<?php echo $themepath; ?>images/ico_7.png" width="18" height="19" alt="edit" /></a>
						<a href="javascript:void(0);" onClick="deleteCategoryTopic('<?php echo $topic_id; ?>','subtopic','<?php echo $action_url; ?>');"><img src="<?php echo $themepath; ?>images/ico_8.png" width="18" height="19" alt="delete" /></a>
					<?php }
					if (user_access('add edit delete subtopic')  && $create_access) { ?>
						<div class="acc_btn"><input type="submit" class="button" value="User Access" onClick="openUserAccess('<?php echo $cat_id; ?>', '<?php echo $topic_id; ?>', '<?php echo $parent_topic_id; ?>', 'subtopic', '<?php echo $action_url . '/subtopic/' . $topic_id; ?>');" /></div>
					<?php } ?>
				</div>
				...
				<div class="cat_list">
					<div class="prod_img" file_id="<?php echo $record->scan_file_id;?>">
						...
						?>
							<img alt="category" title="category" src="<?php echo htmlspecialchars($imagepath); ?>" width="206" height="214" alt="<?php echo htmlspecialchars($record->topic_image); ?>" />
						<?php
					...
					<div class="cat_desc">
						...
						<div class="shw-catdesc-all">
							<?php
							$multidesc = db_query('select description from {topic_multi_description} where region_id=:region_id and topic_id=:topic_id', array(':region_id' => $currentregiontab, ':topic_id' => $topic_id))->fetchColumn();
							if ($multidesc == '') {
						...
						<input type="hidden" value="<?php echo ceil(($parent_data->expiry_date - time()) / (60 * 60 * 24)); ?>" id="max_expiry_edit" />
						<input type="hidden" value="<?php echo $record->expiry_date ? date("m/d/Y", $record->expiry_date) : ''; ?>" id="page_expiry_add" />
						<input type="hidden" value="<?php echo $parent_data->expiry_date ? date("m/d/Y", $parent_data->expiry_date) : ''; ?>" id="page_expiry_edit" />
						<?php
						// PCM live added by Photon-Sivaranjani
						$enable_pcm_live = 0;
						if ($cat_id == 69) {
							$topic_ids = array('133', '251');
						...
						<div class="file_str">
							<ul id="cat-topic-docs" <?php echo $sortul; ?>>
								<?php
								$total_docs = get_docs_totalcount($cat_id, $parent_topic_id, $topic_id, 0, $currentregiontab);
								$end = 5;
								$doc_list = get_docs_intial($cat_id, $parent_topic_id, $topic_id, 0, $currentregiontab);
								$fcount = $fid = 0;
								if ($doc_list) {
									foreach ($doc_list as $file) {
										...
										$ga_name = addslashes(str_replace('"', '', $cat_name . ' > ' . $parent_data->topic_name . ' > ' . $record->topic_name));
										$ga_file_title = addslashes(str_replace('"', '', $file->file_title));

										if ($enable_pcm_live == 0 || $file_type == "pdf") {
								?>
											...
											<li <?php echo $sortclass; ?> id="itemorder_<?php echo $fid; ?>_1">
												<span class="file_ic">
													...
													?>
														<span onClick="downloadUploadDocuments('<?php echo $topic_id; ?>','subtopic', '<?php echo $fid; ?>');track_document_download('<?php print $ga_name; ?>', 'Download', '<?php print $ga_file_title; ?>');" class="<?php echo ($file->expiry_date < (strtotime(date("m/d/Y")))) ? 'doc_title_exp' : 'doc_title_downl'; ?>"><?php echo $file->file_title; ?></span>
													<?php
												...
												<span class="filedwn">
													...
													if ($file->scan_file_status == "SCAN_COMPLETED" || $file->scan_file_status == "") {
													?>
														<a href="javascript:void(0);" onClick="downloadUploadDocuments('<?php echo $topic_id; ?>','subtopic', '<?php echo $fid; ?>');track_document_download('<?php print $ga_name; ?>', 'Download', '<?php print $ga_file_title; ?>');"><img src="<?php echo $themepath; ?>images/ico_6.png" width="18" height="19" alt="download" /></a>
													<?php
													}
													if (user_access('add edit delete subtopic') && user_access('upload file to category topic') && $edit_access) {
													?>
														<a href="javascript:void(0);" onClick="editUploadDocuments('<?php echo $topic_id; ?>','subtopic', '<?php echo $fid; ?>');"><img src="<?php echo $themepath; ?>images/ico_7.png" width="18" height="19" alt="Edit" /></a>
														<a href="javascript:void(0);" onClick="updateDocuments('<?php echo $topic_id; ?>','subtopic', '<?php echo $fid; ?>', 'delete');"><img src="<?php echo $themepath; ?>images/ico_8.png" width="18" height="19" alt="Delete" /></a>
													<?php
													} ?>
												</span>
					...
					<?php } ?>
					<div class="file_page">
						<?php if (user_access('add edit delete subtopic') && user_access('upload file to category topic') && is_vwr_user_role() && $edit_access) { ?>
							<div class="upload_btn"><input type="submit" class="button" value="Upload Document" onClick="openUploadDocuments('<?php echo $topic_id; ?>','subtopic','<?php echo $action_url . '/subtopic/' . $topic_id; ?>', '<?php echo 'cat_id=' . $cat_id . '&top_id=' . $parent_topic_id . '&stop_id=' . $topic_id . '&itop_id=0'; ?>');" /></div>
						<?php }
						if ($fcount) {
						?>
							<div class="fl_pg">
								<input type="hidden" id="current_url" value="<?php echo $action_url . '/subtopic/'; ?>" />
								<?php if ($total_docs > $end) { ?>
									<span class="viewall" id="viewall_docs"><a href="javascript:void(0);" onClick="showDocsPagination('<?php echo $topic_id; ?>', 'subtopic', '<?php echo $fid; ?>', 'viewall');" id="viewall_a">View&nbsp;All</a></span>
								<?php } ?>
								<span class="pge">
									<span id="hide_start_cont" style="display:none;">
										<a class="sh_cursor" onClick="showDocsPagination('<?php echo $topic_id; ?>', 'subtopic', '<?php echo $fid; ?>', 'first');"><img src="<?php echo $themepath; ?>images/older1.jpg" width="7" height="7" /></a>
										<a class="sh_cursor" onClick="showDocsPagination('<?php echo $topic_id; ?>', 'subtopic', '<?php echo $fid; ?>', 'prev');"><img src="<?php echo $themepath; ?>images/prev1.jpg" width="7" height="7" /></a>
									</span>
									<?php if ($enable_pcm_live == 0) { ?>
									...
									<?php if ($total_docs > $end) { ?>
										<span id="hide_last_cont">
											<a class="sh_cursor" onClick="showDocsPagination('<?php echo $topic_id; ?>', 'subtopic', '<?php echo $fid; ?>', 'next');"><img src="<?php echo $themepath; ?>images/next1.jpg" width="7" height="7" /></a>
											<a class="sh_cursor" onClick="showDocsPagination('<?php echo $topic_id; ?>', 'subtopic', '<?php echo $fid; ?>', 'last');"><img src="<?php echo $themepath; ?>images/last1.jpg" width="7" height="7" /></a>
										</span>
									<?php } ?>
								</span>
							</div>
				...
				<?php if ($enable_pcm_live == 0) { ?>
					<div class="drp_cont">
						...
						<ul>
							...
							?>
											<li>
												<a style="cursor:pointer;" onClick="Dropbox_Modalbox('file_upload','<?php echo base_path(); ?>','<?php echo htmlspecialchars($dropbox->id); ?>','<?php echo htmlspecialchars($cat_id); ?>','<?php echo htmlspecialchars($parent_topic_id); ?>','<?php echo htmlspecialchars($topic_id); ?>');" class="drop_ico"><img src="<?php echo $themepath; ?>images/dropbox_up_s.png" width="30" height="28" alt="" class="viewdpb_img" />
				...
				<?php } ?>
				<?php
				$page_display_options = db_query('select display_options from {topic} where topic_id=:topic_id', array(':topic_id' => $topic_id))->fetchColumn();
				...
				<div class="pro_tb">
					<?php
					if ($topic_id != 251) {
						$sub_topics = get_all_topics($cat_id, $topic_id, 1, $currentregiontab, 3);
					}
					if ($sub_topics) {
						foreach ($sub_topics as $topic) {
							...
							if (count($contentregions) > 0) {
								if (check_category_topic_access($cat_id, $topic->topic_id)) {
									if ((!is_vwr_user_role() && ($topic->expiry_date >= (date("m/d/Y")))) || is_vwr_user_role()) {
					?>


										<?php if ($page_display_options == 1) { ?>
											<div class="tb_box" onClick="window.open('<?php echo base_path() . $action_url . '/subtopic/' . base64_encode($topic_id) . '/internaltopic/' . base64_encode($topic->topic_id); ?>','_self');">
												<div class="tb_img" file_id="<?php echo $topic->scan_file_id ;?>">
													...
													?>
														<img title="subtopic" src="<?php echo $tbimagepathnewtopic; ?>" width="88" height="88" alt="<?php echo $topic->topic_image; ?>" />
													<?php
												...
												<h4 title="<?php echo $topic->topic_name; ?>" class="<?php echo ($topic->expiry_date < (strtotime(date("m/d/Y")))) ? 'doc_title_exp' : ''; ?>"><?php echo (strlen($topic->topic_name) > 23) ? substr($topic->topic_name, 0, 20) . '...' : $topic->topic_name; ?></h4>
										...
										<?php if ($page_display_options == 2) { ?>
											<ul style="margin-bottom: 10px;padding-bottom: 10px;">
												...
												<li style="list-style-type:none">
													<h2>
														<a style="color:#495676;cursor:pointer;margin-left:10px" href="<?php echo base_path() . $action_url . '/subtopic/' . $topic_id . '/internaltopic/' . $topic->topic_id; ?>">
					...
					if (user_access('add edit delete internaltopic') && $create_access) { ?>
						<div class="addtpc_btn"><input type="submit" class="button" value="Add Sub-Topic" onClick="addCategoryTopic('<?php echo $topic_id; ?>','internaltopic','<?php echo $action_url . '/subtopic/' . base64_encode($topic_id) . '/internaltopic/add'; ?>');" /></div>
					<?php } ?>
				</div>

				<div class="pro_tb">
					<?php
					$cat_topics = get_all_hyperlinks($cat_id, $parent_topic_id, $topic_id, 0, $currentregiontab);
					if ($cat_topics) {
							...
							<div class="tb_box" style="cursor:auto">
								...
								<h4 title="Asset Management"><?php echo $topic->hyperlink_name; ?>
									...
									<?php if (is_vwr_user_role() && has_page_access('edit')) { ?>
										<a onClick="editHyperlinkCategory('<?php echo $cat_id; ?>','category','<?php echo 'hyperlink/edit/' . $hyperlink_id; ?>','<?php echo 'cat_id=' . $cat_id . '&top_id=' . $parent_topic_id . '&stop_id=' . $topic_id . '&itop_id=0'; ?>');" href="javascript:void(0);"><img src="<?php echo $themepath; ?>images/ico_7.png" width="18" height="19" alt="edit" /></a>
				...
				<?php if (is_vwr_user_role() && has_page_access('edit')) { ?> <div class="addtpc_btn"><input type="submit" class="button" value="Create Hyperlink" onClick="addHyperlink('<?php echo $topic_id; ?>','hyperlink','<?php echo 'category/' . base64_encode($cat_id) . '/hyperlink/topicid/' . base64_encode($parent_topic_id) . '/subtopic/add'; ?>','<?php echo 'cat_id=' . $cat_id . '&top_id=' . $parent_topic_id . '&stop_id=' . $topic_id . '&itop_id=0'; ?>');" /></div>
				<?php } ?>
	...
	<?php
		...
		print theme('noaccess_error_theme', array('action' => '', 'level' => 'topic'));
	}
	?>