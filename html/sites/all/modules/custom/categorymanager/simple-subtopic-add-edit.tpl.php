	<script language="javascript">
		...
		function checkregionstopic(noid) {
			var inputs = document.getElementsByName('region[]');
	...
	<?php
	$parent_topic_id = $topic_id = $cat_id = 0;
	$action_value = 'Add Topic';
	$action_title = 'Create a New Topic';
	$topic_file = '';
	if (arg(1) && is_numeric(arg(1))) {
		$cat_id = arg(1);
	}
	if (arg(3) && is_numeric(arg(3))) {
		$parent_topic_id = arg(3);
	}
	if ($cat_id && arg(4) == 'subtopic' && $action == 'edit' && arg(6) && is_numeric(arg(6))) {
		$topic_id = arg(6);
		$action_value = 'Update Topic';
		$action_title = 'Edit a Topic';
	...
	$parent_topic_name = db_query("SELECT topic_name FROM {topic} where topic_id = :parent_topic_id && category_id=:cat_id", [':parent_topic_id' => $parent_topic_id, ':cat_id' => $cat_id])->fetchColumn();
	?>
	<style type="text/css">
	...
	<div class="pop_cont cat_tpls">
		...
		<div class="poptxt cat_upld">
			...
			<label>Topic Image<span>*</span> :</label>
			<input type="file" accept="image/*" value="<?php echo htmlspecialchars($topic_file); ?>" id="topicfile" name="topicfile" onchange="initiateScan('subtopic', '<?php echo "category/$cat_id/topic/$parent_topic_id/"; ?>')">
			...
			<span class="img_existed">
				...
				?>
					<span id="region_checkboxes">
						<input type="checkbox" id="region_desc<?php echo $i; ?>" onClick="checkregionstopic(<?php echo $i; ?>)" style="float:none !important;margin-left:-26px !important;width:67px !important;" value="<?php echo $rid; ?>" name="region[]" <?php echo $disabled; ?> <?php echo $checkedproperty; ?> />
					</span>
			...
			if ($topic_id) {
				...
				foreach ($regions as $region_id => $regionarray) {
					$result = db_query("select description from {topic_multi_description} where topic_id=:topic_id and region_id=:region_id", [':topic_id' => $topic_id, ':region_id' => $region_id])->fetchField();
		...
		<div class="popBut1">
			...
			<input type="submit" class="button" value="<?php echo $action_value; ?>" onClick="validateAllTopics(<?php echo $topic_id; ?>, <?php echo $cat_id; ?>, 'subtopic', <?php echo $parent_topic_id; ?>, '<?php echo "category/$cat_id/topic/$parent_topic_id/"; ?>');" />
